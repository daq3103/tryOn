...
class MultiSourceAttnProcessor(AttnProcessor):
    ...
    def __call__(
        ...
        attn_scores = torch.baddbmm(
            torch.empty(
                q.shape[0], q.shape[1], k.shape[1], device=q.device, dtype=q.dtype
            ),
            q,
            k.transpose(-1, -2),
            beta=0,
